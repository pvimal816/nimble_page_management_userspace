...
for I in `seq 1 5`; do
	echo "[1]=========starting measurement number $I.=========="
	for MT in ${MULTI}; do
		...
		for BATCH in ${BATCH_MODE}; do
			for METHOD in ${COPY_METHOD}; do
				...
				for N in ${PAGE_LIST}; do
					...
					echo "[1]NUM_PAGES: "${NUM_PAGES}", METHOD: "${PARAM}", BATCH: "${BATCH}", MT: "${MT}

					if [[ "x${I}" == "x1" ]]; then
						ocperf stat -x, -o ocperf_output.tmp -e UNC_M_PMM_RPQ_OCCUPANCY.ALL,UNC_M_PMM_WPQ_OCCUPANCY.ALL,UNC_M_PMM_RPQ_INSERTS,UNC_M_PMM_WPQ_INSERTS numactl -N ${SOURCE_CPU_NODE} -m ${SOURCE_NODE} ./non_thp_move_pages ${NUM_PAGES} ${PARAM} ${BATCH} ${SOURCE_NODE} ${DESTINATION_NODE} 2>./thp_verify/${METHOD}_${MT}_4kb_page_order_${N}_${BATCH} | grep -A 3 "\(Total_cycles\|Test successful\)" > ./stats_4kb/${METHOD}_${MT}_page_order_${N}_${BATCH}
						echo $PERF_EVENT_LIST_STR > ./stats_4kb/${METHOD}_${MT}_page_order_${N}_${BATCH}_perf_stats
					else
						ocperf stat -x, -o ocperf_output.tmp -e UNC_M_PMM_RPQ_OCCUPANCY.ALL,UNC_M_PMM_WPQ_OCCUPANCY.ALL,UNC_M_PMM_RPQ_INSERTS,UNC_M_PMM_WPQ_INSERTS numactl -N ${SOURCE_CPU_NODE} -m ${SOURCE_NODE} ./non_thp_move_pages ${NUM_PAGES} ${PARAM} ${BATCH} ${SOURCE_NODE} ${DESTINATION_NODE} 2>./thp_verify/${METHOD}_${MT}_4kb_page_order_${N}_${BATCH} | grep -A 3 "\(Total_cycles\|Test successful\)" >> ./stats_4kb/${METHOD}_${MT}_page_order_${N}_${BATCH}
						echo "" >> ./stats_4kb/${METHOD}_${MT}_page_order_${N}_${BATCH}_perf_stats
					fi
				
					for i in `seq 0 4`; do
						line=$(cat ocperf_output.tmp | grep -i ${PERF_EVENT_LIST[$i]})
						splittedArr=(${line//,/ })
						currentValue=${splittedArr[0]}
						echo -n "$currentValue," >> ./stats_4kb/${METHOD}_${MT}_page_order_${N}_${BATCH}_perf_stats
						# sums[$i]=$(echo "print(${sums[$i]}+$currentValue)" | python3)
						# sums[$i]=$((${sums[$i]}+$currentValue))
						# echo ${sums[$i]}
					done

					rm ocperf_output.tmp
