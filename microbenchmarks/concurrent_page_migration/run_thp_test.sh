...
CONFIG_ID=("NT_OFF_RPDAA_OFF" "NT_OFF_RPDAA_ON" "NT_ON_RPDAA_OFF" "NT_ON_RPDAA_ON")
NT=(0 0 1 1)
RPDAA=(0 1 0 1)
			...
			for METHOD in ${COPY_METHOD}; do
				...
				for N in ${PAGE_LIST}; do
					...
					for pmemOptimized in ${OPTIMIZATION_CONFIG[@]}; do
						echo "[2]NUM_PAGES: "${NUM_PAGES}", METHOD: "${PARAM}", BATCH: "${BATCH}", MT: "${MT}", pmemOptimized: "${pmemOptimized}
						...
						for i in `seq 0 4`; do
							...
							echo -n "$currentValue," >> ./stats_2mb/${METHOD}_${MT}_page_order_${N}_${BATCH}_${CONFIG_ID[$pmemOptimized]}_perf_stats
