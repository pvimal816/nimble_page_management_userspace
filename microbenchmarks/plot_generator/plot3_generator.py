...
def main():
    ...
    four_thread_non_concurrent_base_page = [
        (1<<i)*1e9/((1<<18)*stats("../thp_page_migration_and_parallel"
            + "/stats_4kb"
            + "/mt_4_4kb_page_order_{0}".format(i)).
        average_stats["Total_cycles"]) for i in page_count
    ]
    ...
    four_thread_non_concurrent_thp_page = [
        (1<<i)*1e9/((1<<9)*stats("../thp_page_migration_and_parallel"
            + "/stats_thp"
            + "/mt_4_2mb_page_order_{0}".format(i)).
        average_stats["Total_cycles"]) for i in page_count
    ]
    ...
    thp_y_data = [thp_migration, four_thread_non_concurrent_thp_page, four_thread_concurrent_thp_page]
    thp_labels = ["THP Migration", "4-Thread Non-concurrent", "4-Thread Concurrent"]
    ...
    base_y_data = [base_page_migration, four_thread_non_concurrent_base_page, four_thread_concurrent_base_page]
    base_page_labels = ["Base Page Migration", "4-Thread Non-concurrent", "4-Thread Concurrent"]
    ...
    axs[0].grid(True)
    axs[1].grid(True)
    sns.lineplot(data=thp_data, x="pagecount", y="bandwidth", hue="migration_type", ax=axs[0]).set(title="Figure 10: Throughput comparison of concurrent page migration(4 threads)", xlabel="Number of Pages", ylabel="Throughput(GB/GCycles)")
    sns.lineplot(data=base_page_data, x="pagecount", y="bandwidth", hue="migration_type", ax=axs[1]).set(title="Figure 10: Throughput comparison of concurrent page migration(4 threads)", xlabel="Number of Pages", ylabel="Throughput(GB/GCycles)")
    plt.tight_layout()
