		...
		for METHOD in ${COPY_METHOD}; do
			...
			for N in ${PAGE_LIST}; do
				...
				for pmemOptimized in ${OPTIMIZATION_CONFIG[@]}; do
					...
					if [[ "x${I}" == "x1" ]]; then
						ocperf stat -x, -o ocperf_temp_output.txt -e UNC_M_PMM_RPQ_OCCUPANCY.ALL,UNC_M_PMM_WPQ_OCCUPANCY.ALL,UNC_M_PMM_RPQ_INSERTS,UNC_M_PMM_WPQ_INSERTS numactl -N ${SOURCE_CPU_NODE} -m ${SOURCE_NODE} ./thp_move_pages ${NUM_PAGES} ${PARAM} ${SOURCE_NODE} ${DESTINATION_NODE} ${pmemOptimized} 2>./thp_verify/${METHOD}_${MT}_2mb_page_order_${N}_${CONFIG_ID[$pmemOptimized]} | grep -A 3 "\(Total_cycles\|Test successful\)" > ./stats_split_thp/${METHOD}_${MT}_split_thp_2mb_page_order_${N}_${CONFIG_ID[$pmemOptimized]}
						echo $PERF_EVENT_LIST_STR > ./stats_split_thp/${METHOD}_${MT}_split_thp_2mb_page_order_${N}_${CONFIG_ID[$pmemOptimized]}_perf_stats
					else
						ocperf stat -x, -o ocperf_temp_output.txt -e UNC_M_PMM_RPQ_OCCUPANCY.ALL,UNC_M_PMM_WPQ_OCCUPANCY.ALL,UNC_M_PMM_RPQ_INSERTS,UNC_M_PMM_WPQ_INSERTS numactl -N ${SOURCE_CPU_NODE} -m ${SOURCE_NODE} ./thp_move_pages ${NUM_PAGES} ${PARAM} ${SOURCE_NODE} ${DESTINATION_NODE} ${pmemOptimized} 2>./thp_verify/${METHOD}_${MT}_2mb_page_order_${N}_${CONFIG_ID[$pmemOptimized]} | grep -A 3 "\(Total_cycles\|Test successful\)" >> ./stats_split_thp/${METHOD}_${MT}_split_thp_2mb_page_order_${N}_${CONFIG_ID[$pmemOptimized]}
						echo "" >> ./stats_split_thp/${METHOD}_${MT}_split_thp_2mb_page_order_${N}_${CONFIG_ID[$pmemOptimized]}_perf_stats
					fi
					...
					for i in `seq 0 4`; do
						...
						echo -n "$currentValue," >> ./stats_split_thp/${METHOD}_${MT}_split_thp_2mb_page_order_${N}_${CONFIG_ID[$pmemOptimized]}_perf_stats
