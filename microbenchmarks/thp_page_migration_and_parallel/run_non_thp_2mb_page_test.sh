		...
		for METHOD in ${COPY_METHOD}; do
			...
			for N in ${PAGE_LIST}; do
				...
				for pmemOptimized in `seq 0 0`; do
					...
					echo "[5]NUM_PAGES: "${NUM_PAGES}", METHOD: "${PARAM}", BATCH: "${BATCH}", MT: "${MT}", pmemOptimized: "${pmemOptimized}
					
					if [[ "x${I}" == "x1" ]]; then
						ocperf stat -x, -o ocperf_temp_output.txt -e UNC_M_PMM_RPQ_OCCUPANCY.ALL,UNC_M_PMM_WPQ_OCCUPANCY.ALL,UNC_M_PMM_RPQ_INSERTS,UNC_M_PMM_WPQ_INSERTS numactl -N ${SOURCE_CPU_NODE} -m ${SOURCE_NODE} ./non_thp_move_pages ${NUM_PAGES} ${PARAM} ${SOURCE_NODE} ${DESTINATION_NODE} ${pmemOptimized} 2>./thp_verify/non_thp_2mb_page_order_${N}_${PMEM_OPTIMIZED_SUFFIX_STR[$pmemOptimized]} | grep -A 3 "\(Total_cycles\|Test successful\)" > ./stats_non_thp/${METHOD}_${MT}_non_thp_2mb_page_order_${N}_${PMEM_OPTIMIZED_SUFFIX_STR[$pmemOptimized]}
						echo $PERF_EVENT_LIST_STR > ./stats_non_thp/${METHOD}_${MT}_non_thp_2mb_page_order_${N}_${PMEM_OPTIMIZED_SUFFIX_STR[$pmemOptimized]}_perf_stats
					else
						ocperf stat -x, -o ocperf_temp_output.txt -e UNC_M_PMM_RPQ_OCCUPANCY.ALL,UNC_M_PMM_WPQ_OCCUPANCY.ALL,UNC_M_PMM_RPQ_INSERTS,UNC_M_PMM_WPQ_INSERTS numactl -N ${SOURCE_CPU_NODE} -m ${SOURCE_NODE} ./non_thp_move_pages ${NUM_PAGES} ${PARAM} ${SOURCE_NODE} ${DESTINATION_NODE} ${pmemOptimized} 2>./thp_verify/non_thp_2mb_page_order_${N}_${PMEM_OPTIMIZED_SUFFIX_STR[$pmemOptimized]} | grep -A 3 "\(Total_cycles\|Test successful\)" >> ./stats_non_thp/${METHOD}_${MT}_non_thp_2mb_page_order_${N}_${PMEM_OPTIMIZED_SUFFIX_STR[$pmemOptimized]}
						echo "" >> ./stats_non_thp/${METHOD}_${MT}_non_thp_2mb_page_order_${N}_${PMEM_OPTIMIZED_SUFFIX_STR[$pmemOptimized]}_perf_stats
					fi
					...
					for i in `seq 0 4`; do
						...
						echo -n "$currentValue," >> ./stats_non_thp/${METHOD}_${MT}_non_thp_2mb_page_order_${N}_${PMEM_OPTIMIZED_SUFFIX_STR[$pmemOptimized]}_perf_stats
