	...
	for MT in ${MULTI}; do
		...
		for METHOD in ${COPY_METHOD}; do
			...
			for N in ${PAGE_LIST}; do
				...
				if [[ "x${I}" == "x1" ]]; then
					ocperf stat -x, -o ocperf_temp_output.txt -e UNC_M_PMM_RPQ_OCCUPANCY.ALL,UNC_M_PMM_WPQ_OCCUPANCY.ALL,UNC_M_PMM_RPQ_INSERTS,UNC_M_PMM_WPQ_INSERTS numactl -N ${SOURCE_CPU_NODE} -m ${SOURCE_NODE} ./non_thp_move_pages ${NUM_PAGES} ${PARAM} ${SOURCE_NODE} ${DESTINATION_NODE} 2>./thp_verify/${METHOD}_${MT}_4kb_page_order_${N} | grep -A 3 "\(Total_cycles\|Test successful\)" > ./stats_4kb/${METHOD}_${MT}_4kb_page_order_${N}
					echo $PERF_EVENT_LIST_STR > ./stats_4kb/${METHOD}_${MT}_4kb_page_order_${N}_perf_stats
				else
					ocperf stat -x, -o ocperf_temp_output.txt -e UNC_M_PMM_RPQ_OCCUPANCY.ALL,UNC_M_PMM_WPQ_OCCUPANCY.ALL,UNC_M_PMM_RPQ_INSERTS,UNC_M_PMM_WPQ_INSERTS numactl -N ${SOURCE_CPU_NODE} -m ${SOURCE_NODE} ./non_thp_move_pages ${NUM_PAGES} ${PARAM} ${SOURCE_NODE} ${DESTINATION_NODE} 2>./thp_verify/${METHOD}_${MT}_4kb_page_order_${N} | grep -A 3 "\(Total_cycles\|Test successful\)" >> ./stats_4kb/${METHOD}_${MT}_4kb_page_order_${N}
					echo "" >> ./stats_4kb/${METHOD}_${MT}_4kb_page_order_${N}_perf_stats
				fi
				...
				for i in `seq 0 4`; do
					...
					echo -n "$currentValue," >> ./stats_4kb/${METHOD}_${MT}_4kb_page_order_${N}_perf_stats
